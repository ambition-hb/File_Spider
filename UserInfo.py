...
class UserInfo:
            ...
            try:
                ...
                if r.status_code == 404:
                    ...
                    return
                        ...
                        f = open('Cookies/user_info_cookies.txt', "r")
                        Lines = f.readlines()
                        if len(Lines) == 0:
                            ...
                            self.change_cookie()
                            # with open('User/answerers_info_loseuser_' + str(self.fileNum) + '.txt','a+') as f1:
                            #     f1.write(str(i + 1) + '\n')
                else:
                    self.delLogger(logger)
                    sys.exit(0)
    ...
    def get_userinfo(self):
        ...
        self.current_proxy = get_IP()
        self.get_cookie()
        # self.answer_info = extract_answerers_info()
        ...
        for i in xrange(self.start, self.end):
            ...
            logfielname = '/log/' + dt + '_user_Info' + '.log'
            logger = Logger(logfilename=logfielname,
                            logname='正在爬取第' + str(i + 1) + '个用户的记录信息').getlog()
            # if self.answer_info.has_key(self.user_id) == 0:
            ...
            try:
                ...
                follower_count = soup.findAll('a', class_='Button NumberBoard-item Button--plain')[1].find('strong',
                                                                                                        class_='NumberBoard-itemValue').get_text()
                # 用户关注的话题数
                topics_count = soup.find('a', class_='Profile-lightItem').find('span', class_='Profile-lightItemValue').get_text()
                ...
                data_plus = {
                    ...
                    "topics_count": int(topics_count.replace(',',''))
                }

            self.mongo.db.User_Info.insert(data_plus)
    ...
    def copycookies(self):
        with open('Cookies/cookies.txt', "r") as f:
            Lines = f.readlines()
        with open('Cookies/user_info_cookies.txt','a+') as f_a:
    ...
    def change_cookie(self):
        with open('Cookies/user_info_cookies.txt', "r") as f:
            Lines = f.readlines()
            dict = eval(Lines[0])
            self.type = dict['type']
            self.headers['Cookie'] = dict['Cookie']
            self.headers['x-ab-param'] = dict['x-ab-param']
        with open('Cookies/user_info_cookies.txt', "w") as f_w:
            for line in Lines[1:]:
                f_w.write(line)
    ...
    def get_createpoint(self):
        self.file = open('CreatePoint/userInfo_createpoint_' + str(self.fileNum) + '.txt','a+')
