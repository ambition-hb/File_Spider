...
class UserTopics:
            ...
            while 1:
                try:
                    ...
                    if r.status_code == 200:
                        ...
                        f = open('Cookies/user_topics_cookies.txt', "r")
                        Lines = f.readlines()
                        if len(Lines) == 0:
                    ...
                    if topics_count == 0:
                        ...
                        self.mongo.db.User_Topics.insert(data_plus)
                        break
                    else:
                        offset = 0
                        while 1:
    ...
    def copycookies(self):
        with open('Cookies/cookies.txt', "r") as f:
            Lines = f.readlines()
        with open('Cookies/user_topics_cookies.txt','a+') as f_a:
    ...
    def change_cookie(self):
        with open('Cookies/user_topics_cookies.txt', "r") as f:
            Lines = f.readlines()
            dict = eval(Lines[0])
            self.type = dict['type']
            self.headers['Cookie'] = dict['Cookie']
            self.headers['x-ab-param'] = dict['x-ab-param']
        with open('Cookies/user_topics_cookies.txt', "w") as f_w:
            for line in Lines[1:]:
                f_w.write(line)
    ...
    def get_createpoint(self):
        self.file = open('CreatePoint/userTopics_createpoint_' + str(self.fileNum) + '.txt','a+')
